...
public class UserController {
    ...
    public ResponseEntity<?> loginUser(
        ...
        if (user.isPresent()) {
            ...
            long expirationTimestamp = (System.currentTimeMillis() + expirationTimeInMillis) / 1000; // Convert to UNIX timestamp
            String token = jwtUtil.generateToken(user.get().getEmail(), expirationTimeInMillis);
            Map<String, Object> response = new HashMap<>();
            response.put("token", token);
            response.put("userId", user.get().getId());
            response.put("expiresAt", expirationTimestamp); // Expiry time as UNIX timestamp
